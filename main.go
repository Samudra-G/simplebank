...
import (
	...
	_ "github.com/Samudra-G/simplebank/doc/statik"
...
func runDBMigration(migrationURL string, dbSource string) {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("cannot create a migration instance: ")
	}
...
func runGrpcServer(config util.Config, store db.Store, taskDistributor worker.TaskDistributor) {
	...
	if err != nil {
		log.Fatal().Msg("cannot build server")
	}
...
func runGatewayServer(config util.Config, store db.Store, taskDistributor worker.TaskDistributor) {
	...
	log.Info().Msgf("HTTP gateway server is running on %s", listener.Addr().String())
