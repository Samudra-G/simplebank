...
func TestTransferTx(t *testing.T) {
	...
	//using go routines is safer
	n := 5
	amount := int64(10)

	errs := make(chan error) // channel receives error
	results := make(chan TransferTxResult) //channel recieves results
	...
	for i := 0; i < n; i++ {
		...
		require.NotContains(t, existed, k) //existed map must not contain k
		existed[k] = true
	}
